...
def parse():
    ...
    return args.xtc, args.tpr, args.gmx, args.b, args.e, args.lblocks_bb, args.lblocks_m 
    

########## User input ##########

xtc, tpr, gmx, b_frame, e_frame, l_blocks_bb, l_blocks_methyl = parse()
...
traj_nopbc = xtc.rstrip('.xtc') + '_prot_nopbc.xtc'
...
rot_trans = xtc.rstrip('.xtc') + '_rot_trans.xtc'
...
# if os.path.isfile('nh.ndx'):
#     print("# nh.ndx already exists. It won't be re-written.")
# else:
print("# Write N-H bonds index file")
nh_res   = []
nh_count = 0
with open("nh.ndx", 'w') as f:
    f.write('[ NH ]\n')
...
pkl.dump( nh_res, open('nh_residues.pkl', "wb") )
                      
""" Prepare index file: methyl C-H bonds """
   
# if os.path.isfile('methyls.ndx'):
#     print("# methyls.ndx already exists. It won't be re-written.")
# else:
print("# Write methyl C-H bonds index file")
methyls_carbons = { 'ALA': ['CB'], 'VAL': ['CG1', 'CG2'], 'THR': ['CG2'], 'ILE': ['CG2', 'CD1'], 'LEU': ['CD1', 'CD2'], 'MET': ['CE'] }

methyl_count = 0                   
with open('methyls.ndx', 'w') as f:
    f.write('[ Methyls ]\n')
...
if os.path.isfile('cc.ndx'):
    print("# cc.ndx already exists. It won't be re-written.")
else:
...
#     bb_tcf  = f'tcf_bb/{bl+1}_{traj_nodir_bb}.xvg'
...
# save_bz2( f'tcf_bb/{traj_nodir_bb}', tcfs_bb )
...
save_bz2( f'tcf_methyl/{traj_nodir_met}', tcfs_methyl )  # save one array containing all methyl TCFs from one trajectory

""" Delete gromacs xvg files to save space and lower file count """       

print( "   # Delete .xvg files" )
if os.path.exists( f'tcf_bb/{traj_nodir_bb}.pbz2'  ):
    for f in glob.glob(f'tcf_bb/*_{traj_nodir_bb}.xvg'):
        os.remove( f )
                        
elif os.path.exists( f'tcf_methyl/{traj_nodir_met}.pbz2'  ):
    for f in glob.glob(f'tcf_methyl/*_{traj_nodir_met}.xvg'):
        os.remove( f )
